...
## 七、开发者工具（第 6 轮新增，本项目自有）

三个对齐项目都没有这一分区，这里按常见的开发者在线工具整理。全部由 `omnitool.dev` 插件离线实现，多数是实时面板。

| 能力 | 状态 | 说明 |
| --- | --- | --- |
| 时间戳转换 | ✅ | `timestamp`：秒 / 毫秒 / 微秒 / 纳秒按位数识别，日期按所选时区解释；ISO 8601、RFC 2822、ISO 周、相对时间 |
| 进制转换 | ✅ | `radix`：2–36 进制与任意大整数（BigInt），8–128 位补码与大小端字节，小数的 float32 / float64 编码 |
| Base64 | ✅ | `base64`：标准 / URL 安全、填充、PEM / MIME 换行、Data URI；解码结果按文件头识别类型，自动模式不会把普通单词误当 Base64 |
| JWT | ✅ | `jwt`：解析、时间状态、HS256/384/512 验签与生成；RS/ES/PS 需要公钥，明确提示暂不支持验证 |
| 哈希与校验 | ✅ | `hash`：MD5、SHA-1/256/512、CRC32、HMAC，sha256sum 格式输出与期望值比对 |
| URL | ✅ | `url`：拆解与查询参数、组件 / URI / 表单三种编码、容错解码 |
| UUID / ID / 密码 | ✅ | `uuid`：UUID v4 / v7、ULID、NanoID、随机密钥与强密码（拒绝采样，显示熵值） |
| 正则表达式 | ✅ | `regex`：匹配、捕获组与命名组、替换预览 |
| 文本对比 | ✅ | `diff`：Myers 行级差异，unified diff，可忽略空白与大小写，也可比较两个文件 |
| Cron | ✅ | `cron`：5 / 6 段、宏、名称缩写，中文说明与接下来的执行时间 |
| 十六进制 | ✅ | `hexdump`：`hexdump -C` 视图与文件类型识别；十六进制 / xxd 文本还原为文件 |
| 命名风格、字符串转义 | ✅ | `naming`、`escape`：10 种命名风格；JSON、JS、HTML、Unicode、SQL、Shell、正则、CSV 转义与反转义 |
| JSON 转类型 | ✅ | `json-types`：TypeScript、Go、Rust serde、Python dataclass、JSON Schema，合并数组元素推断可选与可空 |
| chmod、颜色 | ✅ | `chmod`：八进制与符号双向同步、特殊位；`color`：HEX / RGB / HSL / HSV / OKLCH / CMYK 与 WCAG 对比度 |

**开发者工具小计**（16 项）：✅ 16。

---

## 八、Office 与 CAD：纯浏览器的硬边界
...
- 「✅ 137」与内置工具数 111 不一一对应：一个工具可覆盖多行（如 `watermark` 同时覆盖水印与页码），「PDF 替换颜色」由工作流模板组合现有工具完成。
