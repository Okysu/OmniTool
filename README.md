...
- **111 款开箱即用工具**：全面覆盖 PDF 编排、专业图像转换、音视频剪辑转码、结构化数据解析、开发者工具及本地 AI 推理，深度对标 Stirling-PDF、SnapOtter 等开源工具集的核心功能。
...
| **开发者工具** | 16 款 | 实时面板即改即得、一键复制：时间戳与时区换算、任意进制与补码 / IEEE 754、Base64（含 Data URI 与二进制识别）、JWT 解析验签与生成、MD5 / SHA / CRC32 / HMAC 校验、URL 解析与编解码、UUID v4 / v7、ULID、NanoID 与强密码、正则表达式测试、文本差异（unified diff）、Cron 解析与下次执行时间、十六进制查看与还原、命名风格转换、多格式字符串转义、JSON 转 TypeScript / Go / Rust / Python / JSON Schema、chmod 权限计算、颜色格式与 WCAG 对比度 |
